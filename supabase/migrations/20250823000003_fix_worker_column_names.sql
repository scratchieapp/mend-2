-- Fix worker column names in get_incidents_with_details function
-- The workers table uses given_name and family_name, not first_name and last_name
...
BEGIN
  ...
  SELECT 
    ...
    CAST(w.worker_id AS TEXT) AS worker_employee_number, -- Using worker_id as employee number since column doesn't exist
    -- Employer details
    e.employer_id,
    e.employer_name,
    e.employer_abn,
    ...
    COALESCE(s.city || ', ', '') || 
    COALESCE(s.state || ' ', '') || 
    COALESCE(s.post_code, '') AS site_location,
    -- Department details
    d.department_id,
    d.department_name,
    -- Document count
...
COMMENT ON FUNCTION get_incidents_with_details IS 'Get paginated list of incidents with all related details. Fixed: Uses correct worker column names (given_name, family_name) and constructs site_location from address fields.';