-- Fix get_employer_statistics_direct - remove reference to non-existent w.deleted_at column
-- The workers table doesn't have a deleted_at column
...
BEGIN
    ...
    SELECT 
        ...
        -- Worker count (workers table doesn't have deleted_at column - FIXED)
        (SELECT COUNT(*) FROM workers w 
         WHERE (v_employer_filter IS NULL OR w.employer_id = v_employer_filter))::BIGINT as worker_count,
        ...
        (SELECT COUNT(*) FROM incidents i 
         WHERE (v_employer_filter IS NULL OR i.employer_id = v_employer_filter)
         AND UPPER(i.classification) IN ('LTI', 'LOST TIME INJURY')
         AND i.archived_at IS NULL 
        ...
        (SELECT COUNT(*) FROM incidents i 
         WHERE (v_employer_filter IS NULL OR i.employer_id = v_employer_filter)
         AND (i.incident_status IS NULL OR i.incident_status IN ('Open', 'In Progress', 'In Review', 'Voice Agent'))
         AND i.archived_at IS NULL 
