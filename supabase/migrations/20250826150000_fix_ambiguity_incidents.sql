-- FIX AMBIGUITY: Resolve column reference issues in get_incidents_with_details
-- Date: 2025-08-26

-- Drop existing function
DROP FUNCTION IF EXISTS public.get_incidents_with_details CASCADE;

-- Create fixed function with qualified column references
...
BEGIN
    ...
    IF p_user_id IS NOT NULL THEN
        ...
        BEGIN
            SELECT usc.selected_employer_id::integer
            INTO v_selected_employer_id
            FROM user_session_contexts usc
    ...
    SELECT 
        ...
        -- Employer details (use table alias to avoid ambiguity)
...
-- Add documentation
COMMENT ON FUNCTION public.get_incidents_with_details IS 'FINAL PRODUCTION VERSION: Complete incident details with resolved column ambiguity';