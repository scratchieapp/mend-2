-- Performance Optimization Migration (VERIFIED COLUMNS ONLY)
-- This version only creates indexes on columns that actually exist
...
-- Index for psychosocial factors (JSONB type)
...
-- Index for worker_id (foreign key)
...
-- Composite index for user + employer queries
...
-- Index on employer_id for filtering workers by company
CREATE INDEX IF NOT EXISTS idx_workers_employer_id 
ON public.workers(employer_id);

-- Index on worker names for search
...
-- Sites table HAS employer_id
CREATE INDEX IF NOT EXISTS idx_sites_employer_id 
ON public.sites(employer_id);

-- ============================================
-- DEPARTMENTS TABLE - NO EMPLOYER_ID
-- ============================================
-- Skipping departments table - it doesn't have employer_id column
...
BEGIN
    ...
    -- Handle empty employer list
    IF v_employer_ids IS NULL OR array_length(v_employer_ids, 1) IS NULL THEN
        -- Return zeros for users with no employer access
        RETURN QUERY SELECT 
            0::BIGINT, 0::NUMERIC, 0::NUMERIC, 0::BIGINT, 
            0::BIGINT, 0::BIGINT, 0::NUMERIC, 0::NUMERIC;
        RETURN;
    END IF;
    ...
    SELECT 
        COALESCE(im.total_incidents, 0),
        ROUND(COALESCE(im.avg_lost_time, 0), 1),
        ROUND(COALESCE(im.total_claim_costs, 0), 0),
        COALESCE(im.psychosocial_flags, 0),
        COALESCE(im.missing_hours_reports, 0),
        COALESCE(im.recent_incidents_count, 0),
        CASE 
            WHEN COALESCE(hw.total_hours, 0) > 0 THEN 
                ROUND((COALESCE(im.lti_count, 0) * 1000000.0) / hw.total_hours, 2)
            ELSE 0
        END as ltifr,
        CASE 
            WHEN COALESCE(hw.total_hours, 0) > 0 THEN 
                ROUND((COALESCE(im.mti_count, 0) * 1000000.0) / hw.total_hours, 2)
            ELSE 0
        END as mtifr
...
BEGIN
    ...
    ELSIF p_employer_id IS NOT NULL THEN
        v_employer_ids := ARRAY[p_employer_id];
    ELSE
        -- No filters, return zeros for security
        RETURN QUERY SELECT 
            0::NUMERIC, 0::NUMERIC, 0::BIGINT, 
...
-- Only analyze cost tables if they exist
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'cost_assumptions') THEN
        ANALYZE public.cost_assumptions;
    END IF;
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'cost_calculations') THEN
...
BEGIN
    ...
    RAISE NOTICE '    (VERIFIED AGAINST ACTUAL DATABASE)';
    RAISE NOTICE '================================================';
    RAISE NOTICE '';
    RAISE NOTICE '✅ Created performance indexes on:';
    RAISE NOTICE '   - incidents table (11 indexes)';
    RAISE NOTICE '   - users table (3 indexes)';
    RAISE NOTICE '   - user_employers table (3 indexes)';
    RAISE NOTICE '   - workers table (3 indexes)';
    RAISE NOTICE '   - sites table (1 index)';
    RAISE NOTICE '   - cost tables (2 indexes)';
    RAISE NOTICE '';
    RAISE NOTICE '✅ Created optimized functions:';
    RAISE NOTICE '   - get_dashboard_metrics_fast()';
    RAISE NOTICE '   - get_incidents_summary()';
    RAISE NOTICE '   - get_incident_metrics_for_dashboard()';
    RAISE NOTICE '';
    RAISE NOTICE '⚠️  Note: Departments table has no employer_id';
    RAISE NOTICE '   (This is expected - departments are company-wide)';
    RAISE NOTICE '';
    RAISE NOTICE '📊 EXPECTED PERFORMANCE:';
    RAISE NOTICE '   Before: 5+ minutes (unusable)';
    RAISE NOTICE '   After:  <2 seconds (instant)';
    RAISE NOTICE '   Improvement: 150x+ faster!';
    RAISE NOTICE '';
    RAISE NOTICE '✨ Your dashboard should now load instantly!';
    RAISE NOTICE '================================================';
    RAISE NOTICE '';
END $$;